...
enum GameRenderer {
    ...
    private static func drawFighter(ctx: CGContext, f: Fighter, assets: GameAssets, camX: CGFloat) {
        ...
        if f.kind == .enemy && !f.dead && f.hp < f.maxHp {
            ...
            ctx.fill(CGRect(x: bx - 1, y: by - 1, width: bw + 2, height: 6))
            ctx.setFillColor(UIColor(red: 1, green: 0.18, blue: 0.54, alpha: 1).cgColor)
            ctx.fill(CGRect(x: bx, y: by, width: bw * (f.hp / f.maxHp), height: 4))
        }
    }
    ...
    private static func drawHud(ctx: CGContext, state: GameState) {
        let p = state.player
        // HP panel
        roundRect(ctx, CGRect(x: 16, y: 14, width: 260, height: 58), r: 10,
                  fill: UIColor(red: 0.04, green: 0.02, blue: 0.07, alpha: 0.72),
                  stroke: UIColor(red: 1, green: 0.18, blue: 0.54, alpha: 0.45))

        let nameAttrs: [NSAttributedString.Key: Any] = [
            .font: UIFont.boldSystemFont(ofSize: 14),
            .foregroundColor: UIColor(red: 0.96, green: 0.93, blue: 0.97, alpha: 1)
        ]
        ("JJ" as NSString).draw(at: CGPoint(x: 28, y: 20), withAttributes: nameAttrs)

        roundRect(ctx, CGRect(x: 28, y: 42, width: 200, height: 14), r: 6,
                  fill: UIColor(white: 0, alpha: 0.45), stroke: nil)
        let hpPct = max(0, p.hp / p.maxHp)
        roundRect(ctx, CGRect(x: 28, y: 42, width: 200 * hpPct, height: 14), r: 6,
                  fill: UIColor(red: 1, green: 0.18, blue: 0.54, alpha: 1), stroke: nil)

        // Special
        roundRect(ctx, CGRect(x: 16, y: 78, width: 200, height: 18), r: 8,
                  fill: UIColor(red: 0.04, green: 0.02, blue: 0.07, alpha: 0.72), stroke: nil)
        roundRect(ctx, CGRect(x: 22, y: 82, width: 188, height: 10), r: 5,
                  fill: UIColor(white: 0, alpha: 0.4), stroke: nil)
        let sp = state.specialMeter / 100
        let spColor = sp >= 0.4
            ? UIColor(red: 0.18, green: 0.89, blue: 0.9, alpha: 1)
            : UIColor(red: 0.18, green: 0.89, blue: 0.9, alpha: 0.45)
        roundRect(ctx, CGRect(x: 22, y: 82, width: 188 * sp, height: 10), r: 5, fill: spColor, stroke: nil)
        let spAttrs: [NSAttributedString.Key: Any] = [
            .font: UIFont.systemFont(ofSize: 10),
            .foregroundColor: UIColor(red: 0.66, green: 0.61, blue: 0.72, alpha: 1)
        ]
        ("RIFF SPECIAL" as NSString).draw(at: CGPoint(x: 28, y: 100), withAttributes: spAttrs)

        if state.hasGun {
            roundRect(ctx, CGRect(x: 16, y: 118, width: 150, height: 22), r: 8,
            ...
            ("GUN READY  (F)" as NSString).draw(at: CGPoint(x: 28, y: 122), withAttributes: gunAttrs)
        }
        ...
        if p.combo > 1 {
            ...
            c.draw(at: CGPoint(x: 16, y: 125), withAttributes: ca)
        }
    ...
    static func drawTitle(ctx: CGContext, assets: GameAssets, now: CFTimeInterval) {
        ...
        // Idle character
        let idle = assets.jjIdle!
        let drawH: CGFloat = 220
        let drawW = drawH * (idle.frameW / idle.frameH)
        let frame = Int(now / 0.45) % idle.frameCount
