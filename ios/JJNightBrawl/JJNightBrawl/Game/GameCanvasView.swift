...
final class GameCanvasUIView: UIView {
    ...
    override func didMoveToWindow() {
        ...
        if window != nil {
            bridge.assets.loadAsync { [weak self] in
                // Async load may finish while backgrounded — do not start the link then.
                self?.startLinkIfActive()
            }
        } else {
            stopLink()
        }
    ...
    private func startLinkIfActive() {
        guard window != nil, bridge.assets.ready else { return }
    ...
    @objc private func appForeground() { startLinkIfActive() }
